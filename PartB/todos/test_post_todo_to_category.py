...
# Normal Flow
@scenario("../features/todos/post_todo_to_category.feature", "Add an existing todo to an existing category")
def test_post_todo_to_category():
    pass

# Alternate Flow
@scenario("../features/todos/post_todo_to_category.feature", "Create a new category for the todo")
def test_post_todo_to_category_with_only_title():
    pass

# Error Flow
@scenario("../features/todos/post_todo_to_category.feature", "Add a todo to a nonexisting category")
def test_post_todo_to_category_with_invalid_parameter():
    pass

@when(parsers.parse('the user requests to add an existing todo "{todoId}" to an existing category "{categoryId}"'))
def post_todo_to_category(todoId, categoryId, returnedResponse):
...
def post_todo_to_category_with_only_title(todoId, categoryName, returnedResponse):
    allTodos = requests.get(f'{DEFAULT_API_URL}/todos')
    sortedTodos = sorted(allTodos.json()['todos'], key=lambda todo: todo['id'])
    tId = sortedTodos[int(todoId)]['id']
...
def post_todo_to_category_with_invalid_parameter(todoId, categoryId, returnedResponse):
    allTodos = requests.get(f'{DEFAULT_API_URL}/todos')
    sortedTodos = sorted(allTodos.json()['todos'], key=lambda todo: todo['id'])
    tId = sortedTodos[int(todoId)]['id']
